...
const MINIMUM_WITHDRAWAL_AMOUNT = utils.parseEther(
  process.env.MINIMUM_WITHDRAWAL_AMOUNT || "0.005"
);
const MAXIMUM_DEPOSIT_AMOUNT = utils.parseEther(
  process.env.MAXIMUM_DEPOSIT_AMOUNT || "100"
);
...
describe("Custom Tests", function () {
  ...
  it("Alice 0.08 ETH in L1 -> Alice 0.05 ETH to Bob in L2 -> Bob in L2 -> Alice remaining funds in L1", async () => {
    const { tornadoPool, token, omniBridge } = await loadFixture(fixture);
    const aliceKeypair = new Keypair(); // contains private and public keys

    // STEP 1: Alice deposits 0.08 into tornado pool
    ...
    //STEP 2: Alice sends 0.05 ETH to Bob in L2
    const bobKeypair = new Keypair(); // contains private and public keys
    const bobAddress = bobKeypair.address(); // contains only public key

    // Alice sends 0.05 to Bob
    ...
    await transaction({
      tornadoPool,
      inputs: [aliceDepositUtxo],
      outputs: [bobSendUtxo, aliceSendUtxo],
    });
    ...
    //STEP 3: Bob withdraws all his funds in L2
    const bobShieldAddress = "0xDeaD00000000000000000000000000000000BEEf";
    await transaction({
      tornadoPool,
      inputs: [bobReceiveUtxo],
      outputs: [],
      recipient: bobShieldAddress,
      isL1Withdrawal: false,
    });
    //check bob's Balance
    expect(await token.balanceOf(bobShieldAddress)).to.be.equal(bobSendAmount);

    //STEP 4: Alice withdraws all her remaining funds in L1
    const aliceWithdrawAddress = "0xf9aD3D930AB5df972558636A2B8749e772aC9297";
    await transaction({
      tornadoPool,
      inputs: [aliceSendUtxo],
      outputs: [],
      recipient: aliceWithdrawAddress,
      isL1Withdrawal: true,
    });

    expect(await token.balanceOf(aliceWithdrawAddress)).to.be.equal(0);
